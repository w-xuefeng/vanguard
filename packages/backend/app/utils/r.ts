...
export async function useAuthInterceptor(c: Context) {
  ...
  const tokenExistCheck = await checkException(c, !token, token, 'UNAUTHORIZED');
  if (tokenExistCheck.res) {
    return tokenExistCheck;
  }

  const tokenInfo = await decodeToken(token!);
  const tokenExpiredCheck = await checkException(c, tokenInfo.expired || !tokenInfo.text, token, 'AUTH_EXPIRED');
  if (tokenExpiredCheck.res) {
    return tokenExpiredCheck;
  }

  const user = await UserDAO.getUserByName(tokenInfo.text);
  const tokenInvalidCheck = await checkException(c, !user?.value?.name, token, 'ILLEGAL_ACCESS');
